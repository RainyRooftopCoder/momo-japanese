...
class IAdjectiveApp extends BaseGrammarApp {
    constructor() {
        const config = getModuleConfig('iAdjective');
        super(config);
        ...
        formSelector.innerHTML = `
            <div class="form-selector">
                <div class="selector-header">
                    <h3>활용 형태 선택</h3>
                    <button class="info-modal-btn" onclick="window.iAdjectiveApp.openInfoModal()">
        ...
        display.innerHTML = `
            <div class="adjective-container">
                ...
                <div class="adjective-info">
                    <button class="info-btn" onclick="window.iAdjectiveApp.openInfoModal()">
                        ℹ️ 활용 정보
                    </button>
                </div>
            </div>
    ...
    getConjugatedForm(adjective, formType) {
        ...
        const adjectiveStem = adjective.adjective.slice(0, -1); // い 제거하여 어간 추출

        switch (formType) {
            case '현재형':
                return adjective.adjective; // 원형

            case '과거형':
                return adjectiveStem + 'かった';

            case '현재 부정형':
                return adjectiveStem + 'くない';

            case '과거 부정형':
                return adjectiveStem + 'くなかった';
