...
class Group3VerbApp {
    ...
    async loadVerbData() {
        try {
            const response = await fetch('./data/vocabulary/jlpt/group3_verb_data.json');
            if (!response.ok) {
     ...
     */
    bindEvents() {
        console.log('Binding events for Group3VerbApp');
        ...
        // 클릭 이벤트 핸들러 (this 바인딩)
        this.clickHandler = (e) => {
            // characterScreen이 현재 활성화되고 group3VerbScreen이 있는 상태에서만 이벤트 처리
            const characterScreen = document.getElementById('characterScreen');
            const group3VerbScreen = document.getElementById('group3VerbScreen');

            if (!characterScreen || !group3VerbScreen || !characterScreen.contains(group3VerbScreen)) {
                return;
            }
            ...
            // 정보 모달 버튼 이벤트 - group3VerbScreen 내부에서만
            if (group3VerbScreen.contains(e.target) && e.target.closest('.info-modal-btn[data-action="open-info-modal"]')) {
                console.log('Group3 Info modal button clicked');
                e.preventDefault();
                this.openInfoModal();
            }

            // 모달 닫기 버튼 이벤트 - group3의 모달에서만
            if (group3VerbScreen.contains(e.target) && e.target.closest('.modal-close-btn[data-action="close-info-modal"]')) {
                console.log('Group3 Modal close button clicked');
                e.preventDefault();
                this.closeInfoModal();
            }
        };
        ...
        console.log('Event listeners registered for Group3VerbApp');
...
// 3그룹동사 활용 앱은 navigation.js에서 초기화됩니다