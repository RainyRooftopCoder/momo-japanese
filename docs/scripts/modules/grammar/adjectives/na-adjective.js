...
class NaAdjectiveApp extends window.GrammarShared.BaseGrammarApp {
    constructor() {
        const config = window.GrammarShared.MODULE_CONFIGS.naAdjective;
        super(config);
    ...
    getConjugatedForm(adjective, formType) {
        ...
        switch (formType) {
            case '현재형':
                return adjectiveText + 'だ'; // な형용사 + だ

            case '과거형':
                return adjectiveText + 'だった';

            case '현재 부정형':
                return adjectiveText + 'じゃない';

            case '과거 부정형':
                return adjectiveText + 'じゃなかった';

            case '정중형':
                return adjectiveText + 'です';

            case '과거 정중형':
                return adjectiveText + 'でした';

            default:
                return adjectiveText + 'だ';
        }
    }
    ...
    getFormDescription(formType) {
        ...
        return descriptions[formType] || 'な형용사의 활용형';
    }
